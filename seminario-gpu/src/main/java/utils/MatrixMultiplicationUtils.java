...
public class MatrixMultiplicationUtils
{
    ...
    public static void verification(int[][] matrix) {
	    ...
	    // column 0
        for (int i = 0; i < N; i++) { value2 += matrix[i][0]; }
	    // diagonal
        for (int i = 0; i < N; i++) { value3 += matrix[i][i]; }
	    // line N-1
        for (int j = 0; j < N; j++) { value4 += matrix[N - 1][j]; }
        ...
        double total = WorkloadTimer.timerRead(TimerType.TOTAL.ordinal());
